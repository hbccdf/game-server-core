...
public final class ReconnectingThriftClient {
    ...
    private static class ReconnectingClientProxy<T extends TServiceClient> implements InvocationHandler {
                ...
                try {
                    ...
                    if (e.getTargetException() instanceof TTransportException) {
                        ...
                        if (RESTARTABLE_CAUSES.contains(cause.getType())) {
                            reconnectOrThrowException(baseClient.getClass().getCanonicalName(), baseClient.getInputProtocol().getTransport(), maxRetries, timeBetweenRetries);
                            return method.invoke(baseClient, args);
                        }
                    }
        ...
        private static void reconnectOrThrowException(String transportName, TTransport transport, int maxRetries, long timeBetweenRetries) throws TTransportException {
            int errors = 0;
            try {
                transport.close();
            } catch (Exception e) {
            ...
            while (errors < maxRetries) {
                try {
                    LOG.info("Attempting to reconnect: " + transportName);
                    transport.open();
                    LOG.info("Reconnection successful: " + transportName);
                    break;
                }
                catch (TTransportException e) {
                    LOG.error("Error while reconnecting: " + transportName, e);
                    errors++;

                    if (errors < maxRetries) {
                        try {
                            LOG.info("Sleeping for {} milliseconds before retrying: " + transportName, timeBetweenRetries);
            ...
            if (errors >= maxRetries) {
                throw new TTransportException("Failed to reconnect: " + transportName);
            }
        }
    }
}